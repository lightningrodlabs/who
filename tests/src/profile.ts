...
export default (orchestrator) => {
  ...
  orchestrator.registerScenario('test profile zomes', async (s, t) => {
    ...
    const [alice_who_happ, bob_who_happ] = await installAgents(conductor,  ["alice", "bobbo"])
    const [alice] = alice_who_happ.cells
    const [bobbo] = bob_who_happ.cells

    // Trigger init to run in both zomes
    await alice.call('hc_zome_who', 'list_channels', { category: "General" });
    await bobbo.call('hc_zome_who', 'list_channels', { category: "General" });
